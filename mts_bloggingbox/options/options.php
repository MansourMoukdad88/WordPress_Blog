...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Options', 'bloggingbox' );
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'bloggingbox' );
	...
	*/
	function _enqueue(){
		...
		wp_localize_script( 'nhp-opts-js', 'nhpopts', array(
			...
			'reset_confirm' => __('Are you sure you want to reset ALL options to default?', 'bloggingbox' ),
			'import_confirm' => __('Are you sure you want to import options? All current options will be lost.', 'bloggingbox' ),
			'leave_page_confirm' => __('Settings have changed, you should save them! Are you sure you want to leave this page?', 'bloggingbox' ),
			'child_theme_name_empty' => __('Please enter desired child theme name.', 'bloggingbox' ),
			'import_done' => __('Importing proccess finished!', 'bloggingbox' ),
			'import_fail' => __('Importing proccess failed! Please try again.', 'bloggingbox' ),
			'remove_done' => __('Removal proccess finished!', 'bloggingbox' ),
			'remove_fail' => __('Removal proccess failed! Please try again.', 'bloggingbox' ),
			'reloading_page' => __('Reloading page...', 'bloggingbox' ),
			'import_opt_confirm' => __('Are you sure you want to import demo options? All current options will be lost.', 'bloggingbox' ),
			'import_widget_confirm' => __('Are you sure you want to import demo options and widgets? All current options will be lost and existing widgets deactivated.', 'bloggingbox' ),
			'import_all_confirm' => __('Are you sure you want to import demo?', 'bloggingbox' ),
			'remove_all_confirm' => __("Are you sure you want to remove demo options, content, menus and widgets? Please note that all modifications you've made to imported content will be lost", 'bloggingbox' ),
		) );
	...
	*/
	function _download_options(){
		if(!isset($_GET['secret']) || $_GET['secret'] != md5(AUTH_KEY.SECURE_AUTH_KEY)){wp_die(__( 'Invalid Secret for options use', 'bloggingbox' ));exit;}
		if(!isset($_GET['option'])){wp_die(__( 'No Option Defined', 'bloggingbox' ));exit;}
	...
	function _options_page_html(){
		...
		echo '<div class="wrap"><h2></h2>';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				if ( ! MTS_THEME_WHITE_LABEL ) {
					echo '<a href="https://community.mythemeshop.com/forum/7-theme-support/" class="docsupport" target="_blank"><i class="fa fa-medkit"></i> '.__('Support', 'bloggingbox').'</a>';
				}
					echo '<div class="theme-options-search"><a href="#" class="theme-options-clear-search"><i class="dashicons dashicons-no"></i></a> <input type="text" id="search-theme-options" value="" placeholder="'.esc_attr__('Search Options', 'bloggingbox').'"><a href="#" class="options-search-link"><i class="fa fa-search"></i><span> '.__('Search Options', 'bloggingbox').'</span></a></div>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported"><strong>'.__('Settings Imported!', 'bloggingbox' ).'</strong></div>';
						}else{
							echo '<div id="nhp-opts-save"><strong>'.__('Settings Saved!', 'bloggingbox' ).'</strong></div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn"><strong>'.__('Settings have changed, you should save them!', 'bloggingbox' ).'</strong></div>';
					echo '<div id="nhp-opts-field-errors"><strong><span></span> '.__('error(s) were found!', 'bloggingbox' ).'</strong></div>';

					echo '<div id="nhp-opts-field-warnings"><strong><span></span> '.__('warning(s) were found!', 'bloggingbox' ).'</strong></div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_typography']){
							echo '<li id="typography_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="typography_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="typography_default"><i class="fa fa-text-width"></i> '.'<span class="section_title">'.__('Typography', 'bloggingbox' ).'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><i class="fa fa-sign-in"></i> '.'<span class="section_title">'.__('Import / Export', 'bloggingbox' ).'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> '.__('Dev Mode Info', 'bloggingbox' ).'</a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Import / Export Options', 'bloggingbox' ).'</h2>';

							// presets
							if (!empty($this->args['presets'])) {

								echo '<h4>'.__('Preset Options', 'bloggingbox' ).'</h4>';
								...
								foreach ( $this->args['presets'] as $key => $preset ) {
									echo '<div class="preset" data-nonce="'.$nonce.'" data-demo-id="'.esc_attr($key).'">';
										$imported_flag	= '<span class="demo_part_imported">'.__( '(imported)', 'bloggingbox' ).'</span>';
										...
										if ( ! empty( $preset['demo'] ) ) echo '<a href="'.$preset['demo'].'" target="_blank" class="demo-link">'.__('View demo', 'bloggingbox' ).'</a>';
										...
										echo '<div class="preset-actions-wrap">';
											echo '<button class="button button-secondary import-demo-options-button"'.$disabled_button.'>'.__('Import Theme Options', 'bloggingbox' ).'</button>';
											echo '<button class="button button-secondary import-demo-widgets-button"'.$disabled_button.'>'.__('Import Theme Options & Widgets', 'bloggingbox' ).'</button>';
											if ( !empty( $all_imported ) ) {
												echo '<button class="button button-primary remove-demo-button" data-nonce="'.$nonce.'">'.__('Remove imported data', 'bloggingbox' ).'</button>';
											} else {
												echo '<button class="button button-secondary import-demo-button"'.$disabled_button.'>'.__('Import Theme Options, Widgets & Content', 'bloggingbox' ).'</button>';
											}
										echo '</div>';
								...
								echo '<div id="importing-overlay" class="mfp-hide">';
									echo '<div id="importing-modal">';
										echo '<div id="importing-modal-header">';
											echo '<h2><span class="spinner is-active"></span>' . __( 'Processing, please wait...', 'bloggingbox' ) . '</h2>';
										echo '</div>';
										echo '<div id="importing-modal-content">';
										echo '</div>';
										echo '<div id="importing-modal-footer">';
											echo '<span id="importing-modal-footer-info">'.__( 'Processing, please wait...', 'bloggingbox' ).'</span>';
											echo '<button id="importing-modal-footer-button" class="button button-primary">'.__( 'Ok', 'bloggingbox' ).'</button>';
										echo '</div>';
									echo '</div>';
								echo '</div>';
							...
							echo '<h4>'.__('Import Options', 'bloggingbox' ).'</h4>';

							echo '<p><a href="#" id="nhp-opts-import-code-button" class="button-secondary">' . esc_html__( 'Import Code', 'bloggingbox' ) . '</a></p>';

							echo '<div id="nhp-opts-import-code-wrapper">';

								echo '<div class="nhp-opts-section-desc">';

									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Insert your backup code below and hit Import to restore your site options from a backup.', 'bloggingbox' )).'</p>';
								...
								echo '<div class="nhp-opts-field-wrapper">';
									...
									echo '<input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'bloggingbox' ).'">';
							...
							if( true === $this->args['show_child_theme_opts'] && !is_child_theme() ) {
								...
								echo '<h4>'.__('Child Theme', 'bloggingbox' ).'</h4>';
								echo '<div class="nhp-opts-section-desc">';
									echo '<p class="description">'. __('To create child theme for current theme, enter desired name and hit "Create Child Theme" button', 'bloggingbox' ).'</p>';
								echo '</div>';

								echo '<div class="nhp-opts-field-wrapper">';
									//echo '<label for="nhp-opts-child-name">'.__( 'Child theme name', 'bloggingbox' ).'</label><br />';
									echo '<input type="text" id="nhp-opts-child-name" name="nhp-opts-child-name" value="" />';
									echo '<input type="submit" id="nhp-opts-child-button" name="nhp-opts-child-button" class="button-primary" value="'.__('Create Child Theme', 'bloggingbox' ).'">';
								echo '</div>';
					...
					if(true === $this->args['show_typography']){
						echo '<div id="typography_default_section_group'.'" class="nhp-opts-group-tab">';
							//echo '<h3>'.__('Theme Typography', 'bloggingbox' ).'</h3>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Dev Mode Info', 'bloggingbox' ).'</h2>';
							echo '<div class="nhp-opts-section-desc">';
					...
					echo '<div id="options-search-no-results">'.__('No options found, please refine your query.', 'bloggingbox' ).'</div><!--search: no results-->';
				...
				echo '<div id="nhp-opts-footer">';
					...
					if ( ! empty( $this->sections ) && count( $this->sections ) > 2 ) {
						echo '<input type="submit" name="'.$this->args['opt_name'].'[defaults]" value="'.__('Reset to Defaults', 'bloggingbox' ).'" class="button-secondary" />';
						echo '<input type="submit" name="save" id="savechanges" value="'.__('Save Changes', 'bloggingbox' ).'" class="button-primary" disabled="disabled" />';
					}
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
	...
	*/
	...
	function mts_filesystem_notice() {
		echo '<div class="updated"><p>' . esc_html__( 'You do not have permission to do this.', 'bloggingbox' ) . '</p></div>';
	}
	...
	function mts_list_child_themes() {
		...
		if ( !empty( $child_themes_arr ) ) {
			echo '<p class="description">' . __('Existing child themes for current theme:', 'bloggingbox' ) . '</p>';
			...
			_e('No child themes found for current theme', 'bloggingbox' );
		}
		echo '</div>';
