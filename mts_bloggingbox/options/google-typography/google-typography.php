...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title				= __('Adding A Collection', 'bloggingbox' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'bloggingbox' ).'</p>';
		$adding_content				.= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'bloggingbox' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'bloggingbox' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'bloggingbox' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'bloggingbox' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'bloggingbox' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'bloggingbox' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'bloggingbox' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'bloggingbox' ).'</li><li><b>'.__('Font Variant', 'bloggingbox' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'bloggingbox' ).'</li><li><b>'.__('Font Size', 'bloggingbox' ).'</b> - '.__('The size you would like this font to be.', 'bloggingbox' ).'</li><li><b>'.__('Font Color', 'bloggingbox' ).'</b> - '.__('The color you\'d like to use for this font.', 'bloggingbox' ).'</li><li><b>'.__('CSS Selectors', 'bloggingbox' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'bloggingbox' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'bloggingbox' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'bloggingbox' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'bloggingbox' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'bloggingbox' ).':</b> '.__('#selector', 'bloggingbox' ).'</li><li><b>'.__('Classes:', 'bloggingbox' ).':</b> '.__('.selector', 'bloggingbox' ).'</li><li><b>'.__('HTML Tags', 'bloggingbox' ).':</b> '.__('span', 'bloggingbox' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'bloggingbox' ).':</b> '.__('#selector span.date', 'bloggingbox' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'bloggingbox' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title						= __('Theme Typography', 'bloggingbox' );
		$loading					= __('Loading Your Collections', 'bloggingbox' );
		$add_new					= __('Add New Collection', 'bloggingbox' );
		$reset						= __('Reset Collections', 'bloggingbox' );
		$preview_text   			= __('Type in some text to preview...', 'bloggingbox' );
		$collection_title 			= __('New Collection', 'bloggingbox' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'bloggingbox' );
		//$font_family_title  	= __('Font family...', 'bloggingbox' );
		$std_font_family_title  	= __('Standard Fonts', 'bloggingbox' );
		$google_font_family_title  	= __('Google Fonts', 'bloggingbox' );
		$font_variant_title 		= __('Variant...', 'bloggingbox' );
		$font_size_title 			= __('Size...', 'bloggingbox' );
		$backup_font_title			= __('Fallback:', 'bloggingbox' );
		$css_selectors_title		= __('CSS Selectors:', 'bloggingbox' );
		$additional_css_title		= __('Additional CSS:', 'bloggingbox' );
		$delete_button_text			= __('Delete', 'bloggingbox' );
		$save_button_text			= __('Save', 'bloggingbox' );
		
		$welcome_title 				= __('Theme Typography', 'bloggingbox' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'bloggingbox' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'bloggingbox' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'bloggingbox' );
		$step_1_desc				= __('Choose from any of the 600+ Google Fonts.', 'bloggingbox' );
		$step_2_title 				= __('2. Customize It', 'bloggingbox' );
		$step_2_desc				= __('Pick a size, variant, color and more.', 'bloggingbox' );
		$step_3_title 				= __('3. Attach It', 'bloggingbox' );
		$step_3_desc				= __('Attach your font to any CSS selector(s).', 'bloggingbox' );
		$year						= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'bloggingbox' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'bloggingbox' );
		...
		$character_sets = array(
			'latin' 		=> __('Latin', 'bloggingbox' ),
			'latin-ext' 	=> __('Latin Extended', 'bloggingbox' ),
			'cyrillic' 		=> __('Cyrillic', 'bloggingbox' ),
			'cyrillic-ext'	=> __('Cyrillic Extended', 'bloggingbox' ),
			'greek'			=> __('Greek', 'bloggingbox' ),
			'greek-ext'		=> __('Greek Extended', 'bloggingbox' ),
			'vietnamese'	=> __('Vietnamese', 'bloggingbox' ),
			'khmer'			=> __('Khmer', 'bloggingbox' ),
			'devanagari'	=> __('Devanagari', 'bloggingbox' ),
		);
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'bloggingbox' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'bloggingbox' )
			));
