...
function mts_inner_sidebar_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $custom_sidebar).'>-- '.__('Default', 'bloggingbox' ).' --</option>';
	...
	echo '<option value="mts_nosidebar" '.selected('mts_nosidebar', $custom_sidebar).'>-- '.__('No sidebar --', 'bloggingbox' ).'</option>';
	echo '</select><br />';
	
	// Select single layout (left/right sidebar)
	echo '<div class="mts_sidebar_location_fields">';
	echo '<label for="mts_sidebar_location_default" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_default" value=""'.checked('', $sidebar_location, false).'>'.__('Default side', 'bloggingbox' ).'</label>';
	echo '<label for="mts_sidebar_location_left" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_left" value="left"'.checked('left', $sidebar_location, false).'>'.__('Left', 'bloggingbox' ).'</label>';
	echo '<label for="mts_sidebar_location_right" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_right" value="right"'.checked('right', $sidebar_location, false).'>'.__('Right', 'bloggingbox' ).'</label>';
	echo '</div>';
...
function mts_inner_posttemplate_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $posttemplate).'>'.__('Default Post Template', 'bloggingbox' ).'</option>';
	echo '<option value="parallax" '.selected('parallax', $posttemplate).'>'.__('Parallax Template', 'bloggingbox' ).'</option>';
	echo '<option value="zoomout" '.selected('zoomout', $posttemplate).'>'.__('Zoom Out Effect Template', 'bloggingbox' ).'</option>';
	echo '</select><br />';
}
...
function mts_add_postheader_metabox() {
	...
	foreach ($screens as $screen) {
		add_meta_box(
			...
			__('Header Animation', 'bloggingbox' ),	// title
...
function mts_inner_postheader_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $postheader).'>'.__('None', 'bloggingbox' ).'</option>';
	echo '<option value="parallax" '.selected('parallax', $postheader).'>'.__('Parallax Effect', 'bloggingbox' ).'</option>';
	echo '<option value="zoomout" '.selected('zoomout', $postheader).'>'.__('Zoom Out Effect', 'bloggingbox' ).'</option>';
	echo '<option value="traditional" '.selected('traditional', $postheader).'>'.__('Traditional', 'bloggingbox' ).'</option>';
	echo '</select><br />';
}
