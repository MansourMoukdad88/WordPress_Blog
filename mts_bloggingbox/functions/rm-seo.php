...
class MTS_RMU {
    ...
    private function __construct( $config = array() ) {
        $config_defaults = array(
            ...
            'link_label_install'      => __( 'Try it for FREE!', 'bloggingbox' ),
            'link_label_activate'     => __( 'Click here to activate it.', 'bloggingbox' ),

            // Add tab in MTS Theme Options.
            'add_theme_options_tab'   => true,
            'theme_options_notice'    => '<span style="display:block;line-height:1.8;margin-bottom:20px;padding-left:3%;margin-top:-40px;">' . sprintf( __( '%1$s is a revolutionary SEO product that combines the features of many SEO tools. Its features give you the power of an entire SEO team with just a few clicks.', 'bloggingbox' ), '<a href="https://rankmath.com/wordpress/plugin/seo-suite/" target="_blank">Rank Math SEO</a>' ) . ' @CTA' . '</span>',
            ...
            'metabox_notice_install'  => sprintf( __( 'The new %1$s plugin will help you rank better in the search results.', 'bloggingbox' ), '<a href="https://rankmath.com/wordpress/plugin/seo-suite/" target="_blank">Rank Math SEO</a>' ) . ' @CTA',

            /* Translators: %s is CTA, e.g. "Try it now!" */
            'metabox_notice_activate' => sprintf( __( 'The %1$s plugin is installed but not activated.', 'bloggingbox' ), '<a href="https://rankmath.com/wordpress/plugin/seo-suite/" target="_blank">Rank Math SEO</a>' ) . ' @CTA',

            // Add a message in Yoast & AIO metaboxes.
            'show_competitor_notice'  => true,
            'competitor_notice'       =>
                '<span class="dashicons dashicons-lightbulb"></span>
                <span class="mts-ctad-question">' . __( 'Did you know?', 'bloggingbox' ) . ' </span>
                <span class="mts-ctad">' . sprintf( __( 'The new %1$s plugin can make your site load faster, offers more features, and can import your current SEO settings with one click.', 'bloggingbox' ), '<a href="https://rankmath.com/wordpress/plugin/seo-suite/" target="_blank">Rank Math SEO</a>' ) . '</span>' . '@CTA',
        );
    ...
    public function dashboard_widget_output( $post, $callback_args ) {
        ...
        <div class="rmu-dashboard-panel">
            <a class="rmu-dashboard-panel-close" id="rmu-dashboard-dismiss" href="<?php echo esc_url( add_query_arg( 'rmu-dashboard', '1' ) ); ?>"><?php echo esc_attr( 'Dismiss', 'bloggingbox' ); ?></a>
    ...
    public function get_install_link( $class = '', $label = '' ) {
        ...
        return '<a href="' . $install_link . '" class="thickbox ' . esc_attr( $class ) . '" title="' . esc_attr__( 'Rank Math SEO', 'bloggingbox' ) . '">' . $label . '</a>';
    }
    ...
    public function add_theme_options_seo_tab( $sections ) {
        ...
        $sections[] = array(
            ...
            'title'  => __( 'SEO', 'bloggingbox' ),
            'desc'   => '<p class="description">' . __( 'Fix all your SEO issues with just a few clicks. Comply with all the latest Google’s guidelines.', 'bloggingbox' ) . '</p>',
            'fields' => array(
                'seo_message' => array(
                    'id'    => 'mts_seo_message',
                    'type'  => 'info',
                    'title' => '<a href="https://rankmath.com/wordpress/plugin/seo-suite/" target="_blank" class="rm-logo">' . $this->get_logo_svg() . '</a>',
                    'desc'  => strtr( $this->get_setting( 'theme_options_notice' ), array( '@CTA' => $this->get_install_or_activate_link() ) ) . $didyouknow . $this->get_install_or_activate_link( 'button button-primary', __( 'Install Now', 'bloggingbox' ), __( 'Activate It', 'bloggingbox' ) ) . $this->get_notice_css(),
                ),
            ),
        );
        return $sections;
